document.addEventListener('DOMContentLoaded', function(){
  ...
  // find out if the active element is inside a submenu. If so, open the submenu (accordion only)
  if (active) {
...
const addOffClick = (e) => {
  const offClick = (evt) => {
    if (e !== evt) {
      ...
      // find the open menu item and close it
      var open = document.querySelector('.us_menu.horizontal > .dropdown.open');
      if (open) {
